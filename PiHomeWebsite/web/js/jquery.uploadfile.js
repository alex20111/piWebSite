...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
	   ...
	   this.startUpload = function () {
        ...
        this.cancel = function (fName) {
            $("." + s.cancelButtonClass).each(function (i, items) {
            	
                if($(this).hasClass(obj.formGroup) && $(this).attr("filenameid") == fName ){
                	$(this).click();
                	return false;
                }
            });
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
				...
				pd.filename.html(fileNameStr);
				pd.cancel.attr('filenameid', files[i].name);
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span class='" + s.errorClass + " fa fa-times-circle' style='color:red'   >ERROR: " + msg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
